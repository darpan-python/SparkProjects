...
class Intializer(sparkIntializer):
    ...
    def flow(self, df):
        if isinstance(self.stream, type(df)):
            print("True")
    ...
    def process_df(self,df, epochId):
        import datetime
        print(epochId)
        print(datetime.datetime.now())
        row = df.select(df.Ledger).take(1)[0]["Ledger"]
        print(row)

        if (row =="Sap"):
            df.write.mode('append').csv('mycsvdarpan.csv', header="true")
        pass
    ...
    def spark_sink1(self, lines):
        query = lines \
            ...
            .option("checkpointLocation", "/home/darpan/snap5") \
            .start() \
            .awaitTermination()
    pass
